...
class Database:
    ...
    def get_publication_id_by_name(self, publication_name):
        ...
        try:
            cursor = self.get_cursor()
            cursor.execute("SELECT id FROM publications WHERE name = ?", (publication_name,))
            result = cursor.fetchone()
    ...
    def create_article(self, title, content, publication_id, article_date, original_filename=None):
        """Create a new article"""
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            cursor.execute("""
            INSERT INTO articles (title, content, publication_id, article_date, original_filename)
            VALUES (?, ?, ?, ?, ?)
            """, (title, content, publication_id, article_date, original_filename))
            conn.commit()
            return cursor.lastrowid
        except sqlite3.Error as e:
    ...
    def get_articles_by_date_and_publication(self, publication_id, article_date):
        """Get articles filtered by date and publication"""
        try:
            cursor = self.get_cursor()
            cursor.execute("""
            ...
            WHERE a.publication_id = ? AND a.article_date = ?
            ORDER BY a.created_at DESC
            """, (publication_id, article_date))
            return [dict(row) for row in cursor.fetchall()]
        except sqlite3.Error as e:
    ...
    def update_article_inclusion(self, article_id, included):
        """Update whether an article is included in the context"""
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            cursor.execute("""
            UPDATE articles SET included = ? WHERE id = ?
            """, (1 if included else 0, article_id))
            conn.commit()
            return cursor.rowcount > 0
        except sqlite3.Error as e:
            print(f"Error updating article inclusion: {e}")
            conn.rollback()
            return False
    ...
    def get_included_articles_content(self, publication_id, article_date):
        """Get the content of all included articles for a specific date and publication"""
        try:
            cursor = self.get_cursor()
            cursor.execute("""
            SELECT title, content 
            FROM articles 
            WHERE publication_id = ? AND article_date = ? AND included = 1
            ORDER BY created_at DESC
            """, (publication_id, article_date))
            articles = cursor.fetchall()
            
            # Combine all article content with titles as headers
            combined_content = ""
            ...
            print(f"Error fetching included articles: {e}")
            return ""
